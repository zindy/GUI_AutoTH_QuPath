/* GUI for using Auto-Threshold methods
 * 
...
Stage buildStage() {
   ...
   //Button 1
    ...
    btnRunParents.setOnAction {e ->
        ...
        for(object in objects) {
          ...
          autoThreshold(object, comboChannel.getValue(), downsampleAT, combo.getValue(), floorValueSpinner.getValue(), comboOutput.getValue(), comboResolutionsClassifier.getValue(),comboPrefilter.getValue(),sigmaSpinner.getValue(), comboAvobe.getValue(), comboBelow.getValue(), minValueSpinner.getValue(), minHoleSpinner.getValue(), postProcessingOptions.join(','))
   ...
   pane.add(runningTittle2, 0,row, 2, 1)
    ...
    PaneTools.setToExpandGridPaneWidth(combo, btnSelected,comboChannel,comboAvobe,comboBelow,comboParent,comboResolutions,comboResolutionsClassifier,comboOutput,btnRunParents)

    def scene = new Scene(pane)
    def stage = new Stage()
    stage.setTitle("Auto threshold GUI")
...
/* FUNCTIONS */
def autoThreshold(annotation, channel, thresholdDownsample, thresholdMethod, thresholdFloor, output, classifierDownsample,prefilterType,prefilterSigma, classBelow, classAbove, minArea, minHoleArea, classifierObjectOptions) {
